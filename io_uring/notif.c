...
/* 
 * Completes notification requests in the task work queue.
 * Processes each notification in the chain, handles zero-copy flags, and frees resources.
 */
...
/* 
 * Handles completion of user buffer operations, tracks zero-copy usage.
 * Updates internal flags based on operation outcome and schedules task work completion.
 */
...
/* 
 * Links socket buffer with user buffer for zero-copy networking operations.
 * Verifies compatibility between buffers and builds a chain of notification nodes.
 */
...
/* 
 * Defines operations for user buffer handling in zero-copy transfer.
 * Connects callback functions for buffer completion and linking.
 */
static const struct ubuf_info_ops io_ubuf_ops = {
...
/* 
 * Creates a new notification request within the io_uring context.
 * Initializes all required state for zero-copy operations and reference tracking.
 */
